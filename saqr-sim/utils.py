...
# Hop delays in ns
LINK_DELAY_TOR = range(6000, 18000)
LINK_DELAY_SPINE = range(5000, 13000) 
LINK_DELAY_CORE = range(5000, 13000)


NUM_TASK_PER_WORKER = 120
...
def write_to_file(working_dir, metric, policy, load, distribution, results, run_id, cluster_id=None, is_colocate=False):
    if cluster_id != None:
        ...
        filename = policy + '_' + distribution + '_' +metric +  '_' + str(load) + '_r' + run_id + '.csv'
